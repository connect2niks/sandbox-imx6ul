...
const char *g_tool_version = "1.0.0"; /**< Global holds tool version */
...
static uint32_t g_nb_srk_table = 1;
 ...
 */
static void validate_header(struct ahab_container_header_s *header,
                            long file_size)
{
    if (header == NULL)
    {
    ...
    {
        fprintf(stderr, "Error: Unsupported container version: %d\n",
                header->version);
        exit(EXIT_FAILURE);
    }
    ...
    {
        fprintf(stderr, "Error: Invalid container tag: 0x%X\n", header->tag);
        exit(EXIT_FAILURE);
    }
    ...
    {
        fprintf(stderr, "Error: Length exceeds file size\n");
        exit(EXIT_FAILURE);
    }
}
 ...
 * @param[in] header Pointer to the container header structure.
 */
static void display_header(struct ahab_container_header_s *header)
{
    uint8_t *tag = NULL;
    ...
    parse_container_header_flags(header->flags);
 ...
 */
static void verify_image(FILE *file, struct ahab_container_image_s *image,
                         int index,
                         struct ahab_container_header_s *container_header)
{
    ...
    if (image->image_size > g_size)
    {
        fprintf(stderr,
                "Error: Invalid image size (image_size: %u, g_size: %zu).\n",
                image->image_size, g_size);
        return;
    }
...
static void parse_srk_table_array(FILE *file, long srk_table_array_offset)
{
    ...
    g_nb_srk_table = srk_table_array.number_srk_table;

    increase_indent();

    /* Parse each SRK table */
    offset = srk_table_array_offset + sizeof(srk_table_array);
    for (i = 0; i < g_nb_srk_table; i++)
    {
        /* Parse SRK table */
 ...
 */
static void parse_signature(FILE *file, long signature_offset, long data_offset,
                            ...
                            struct ahab_srk_record *srk_record)
{
    ...
    if ((size_t) signature_offset > g_size || (size_t) data_offset > g_size)
    {
        fprintf(stderr,
                "Error: Offset exceeds file size (g_size: %zu, "
                "signature_offset: %ld, data_offset: %ld).\n",
                g_size, signature_offset, data_offset);
        return;
    }

    if (data_length == 0 || data_length > g_size)
    {
        fprintf(stderr,
                "Error: Invalid data length (data_length: %zu, g_size: %zu).\n",
                data_length, g_size);
        return;
    }

    if ((size_t) data_offset + data_length > g_size)
    {
        fprintf(stderr,
                "Error: Data range exceeds file bounds (data_offset: %ld, "
                "data_length: %zu, g_size: %zu).\n",
                data_offset, data_length, g_size);
        return;
    }

    /* Validate signature offset against g_offset */
    if ((size_t) signature_offset < g_offset ||
        (size_t) signature_offset >= g_size)
    {
        fprintf(stderr,
                "Error: Signature offset is outside valid range (g_offset: "
                "%zu, g_size: %zu, signature_offset: %ld).\n",
                g_offset, g_size, signature_offset);
        return;
    }
 ...
 */
static void parse_certificate_v1(FILE *file, long certificate_offset,
                                 struct ahab_certificate *cert)
{
    ...
    {
        parse_signature(file, certificate_offset + cert->v1.signature_offset,
 ...
 * @param[out] cert Pointer to the certificate structure to be filled.
 */
static void parse_certificate_v2(FILE *file, long certificate_offset,
                                 struct ahab_certificate *cert)
{
    int i = 0;
    long offset = 0;
    ...
    if (cert->v2.signature_offset != 0)
    {
        ...
        {
            parse_signature(
                file, certificate_offset + cert->v2.signature_offset + offset,
 ...
 * @param[out] cert Pointer to the certificate structure to be filled.
 */
static void parse_certificate(FILE *file, long certificate_offset,
                              struct ahab_certificate *cert)
{
    ...
    {
        parse_certificate_v1(file, certificate_offset, cert);
    }
    else if (certificate_version == CERTIFICATE_VERSION_2)
    {
        parse_certificate_v2(file, certificate_offset, cert);
    }
    else
    {
        fprintf(stderr, "Error: Unsupported certificate version: %d\n",
...
static void parse_blob(FILE *file, long blob_offset)
{
    ...
    print_with_indent("Key Size: 0x%X\n", blob.size);
    switch (blob.size)
    {
    ...
    print_with_indent("Algorithm: 0x%X\n", blob.algorithm);
 ...
 */
static void parse_signature_block(FILE *file, long signature_block_offset)
{
    ...
    for (i = 0; i < g_nb_srk_table; i++)
    {
        ...
        parse_signature(
            file, signature_block_offset + sig_block.signature_offset + offset,
            g_offset,
            signature_block_offset + sig_block.signature_offset - g_offset, i,
            &g_srk_tables[i].sig, srk_record);
 ...
 */
static void parse_file(const char *filename, long offset)
{
    FILE *file = NULL;
    long file_size = 0;
    long image_offset = 0;
    ...
    {
        fprintf(stderr, "Error: Invalid number of images %d in container\n",
                header.nrImages);
        fclose(file);
        exit(EXIT_FAILURE);
    }
        ...
        header.tag == V2X_CONTAINER_HEADER_TAG)
    {
        ...
        for (i = 0; i < header.nrImages; i++)
        {
            ...
            {
                fprintf(stderr, "Error: Image size exceeds file size\n");
                fclose(file);
                exit(EXIT_FAILURE);
            }
            ...
            verify_image(file, &image, i, &header);
        ...
        {
            parse_signature_block(file, offset + header.signature_block_offset);
        }
    }
    else
    {

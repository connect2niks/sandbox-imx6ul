                                  ...
                                  size_t *out_size)
{
    ...
    size_t key_data_size = 0;        /**< Size of key data in bytes */
    ...
    for (size_t i = 0; i < num_tables; i++)
    {
        ...
        /* Calculate size of SRK Data based on key data from SRK record 0 */
        key_data_size = ahab_srk_key_data_size(
            ((struct ahab_container_srk_s
                  *) (tables[i].table +
                      sizeof(struct ahab_container_srk_table_s)))
                ->key_size_or_curve);
        total_size += sizeof(struct ahab_container_srk_data_s) + key_data_size;
    }
...
digest_alg_tag(const char *digest_alg)
{
    ...
    if (digest_alg)
    {
        ...
        else if (strncasecmp((digest_alg), HASH_ALG_SHAKE128_256,
                             sizeof(HASH_ALG_SHAKE128_256)) == 0)
        {
            algorithm = HAB_ALG_SHAKE128_256;
        }
        else if (strncasecmp((digest_alg), HASH_ALG_SHAKE256_512,
                             sizeof(HASH_ALG_SHAKE256_512)) == 0)
        {
...
/*--------------------------
  ahab_srk_key_data_size
---------------------------*/
size_t ahab_srk_key_data_size(uint8_t key_size_or_curve)
{
    size_t size = 0;

    switch (key_size_or_curve)
    {
        case SRK_PRIME256V1:
            size = 32 * 2;
            break;
        ...
        case SRK_RSA4096:
            size = 4096 / 8;
            break;
        case SRK_MLDSA65: /* or SRK_DILITHIUM3 */
            size = 1952;
            break;
        case SRK_MLDSA87: /* or SRK_DILITHIUM5 */
            size = 2592;
            break;
    }

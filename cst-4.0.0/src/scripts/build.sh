...
#   -no-pkcs11: Build CST without PKCS#11 OpenSSL engine
#   -no-pqc: Build CST without PQC support
#   -no-usb: Build CST without USB support
...
if $PQC_SUPPORT; then
    ...
    if [ -z "$oqsprovider_DIR" ]; then
        if [ ! -f "$OQSPROVIDER_INSTALL_DIR/lib/oqsprovider.a" ]; then
            ...
            # Version 0.6.1 does not have means to exclude tests & examples from build
            sed -i '/(test)/s/^/# /' CMakeLists.txt && \
            sed -i '/(examples)/s/^/# /' CMakeLists.txt && \
